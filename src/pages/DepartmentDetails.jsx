      ...
      <section className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-3">
          <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.08 }} className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm md:col-span-2">
            ...
            <p className="mt-4 max-w-xl leading-7 text-slate-600">Explore the people, academic information and research interests connected with this department.</p>
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.34 }} className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">Browse the profiles below to learn more about the people connected with this department.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-6xl px-6 py-12">
          <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-8">
            ...
            <p className="mt-2 text-slate-500">Showing {departmentPeople.length} records</p>
          </motion.div>

          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {departmentPeople.map((person) => <FacultyCard key={person.id} person={person} onViewProfile={setSelectedPerson} />)}
          </div>
        </div>
      </section>
